...
st.markdown(
    "**Track product prices from e-commerce websites with AI-powered monitoring and notifications**"
)
...
with st.sidebar:
    ...
    st.info(
        "API keys are securely loaded from environment variables (`api.env`).\n\n"
        "No keys are exposed in the UI."
    )

    st.markdown("#### ⚙️ Configuration")
    max_products = st.slider(
        "Max Products to Track",
        min_value=1,
        max_value=10,
        value=5
    )
    ...
    notification_method = st.selectbox(
        "Notification Method",
        ["None", "Twilio SMS", "Twilio WhatsApp"]
    )
